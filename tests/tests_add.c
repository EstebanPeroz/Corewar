...
Test(corewar, test_handle_add_basic)
{
    ...
    handle_add(&params);
    cr_assert_eq(champ->registers[2], 42, "add: expected r3 == 42 but got %d", champ->registers[2]);
    cr_assert_eq(champ->carry, 0, "add: expected carry == 0 but got %d", champ->carry);
}

Test(corewar, invalid_type, .init = redirect_all_std)
{
    virtual_machine_t *vm = init_virtual_machine(0, 0);
    champions_t *champ = calloc(1, sizeof(champions_t));
    champ->registers[0] = 10;
    ...
    int res = handle_add(&params);
    cr_assert_eq(res, EXIT_FAILURE);
    cr_assert_eq(champ->carry, 0);
}

Test(corewar, not_valid_register, .init = redirect_all_std)
{
    virtual_machine_t *vm = init_virtual_machine(0, 0);
    champions_t *champ = calloc(1, sizeof(champions_t));
    champ->registers[0] = 10;
